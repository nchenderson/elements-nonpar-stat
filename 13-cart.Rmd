...
tmp <- WBSS(y=yy, x=xx)
plot(tmp$x, tmp$y, main="Within-Bin Sum of Squares by Splitting Point", ylab="WBSS", xlab="split point", las=1)
```
...
```{r, dectree-basic, echo=FALSE, fig.cap="Binary decision tree representing a regression function estimate with 4 bins. The 4 bins here are [0, 0.6), [0.6, 0.74), [0.74, 0.89), [0.89, 1)."}
library(rpart)
library(rpart.plot)
