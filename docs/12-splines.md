...
* Let us go back to the piecewise cubic model shown in Figure ?.
...
* So, for the example that we have in Figure ? with the knots $u_{1}$ and $u_{2}$, we need to enforce the additional four constraints:
\begin{eqnarray}
...
* Hence, the piecewise cubic model that has the two continuity constraints () and the four first and second derivative constraints
...
* The B-spline functions are a basis for the set of cubic splines. 

### Estimating the Coefficients of a Cubic Spline

* When fitting a cubic spline, we assume that the knots $u_{1} < u_{2} < \ldots < u_{q}$ are fixed first.
...
* Finding the coefficients $\beta_{1}, \ldots, \beta_{q+4}$ to minimize this sum-of-residuals-squared criterion
can be viewed as solving a regression problem with response vector $\mathbf{Y} = (Y_{1}, \ldots, Y_{n})$ and
"design matrix"
\begin{equation}
\mathbf{X}_{\varphi} = \begin{bmatrix} \varphi_{1}(x_{1}) & \varphi_{2}(x_{1}) & \ldots & \varphi_{q+4}(x_{1}) \\ \varphi_{1}(x_{2}) & \varphi_{2}(x_{2}) & \ldots & \varphi_{q+4}(x_{2}) \\ \vdots & \vdots & \ddots & \vdots \\ \varphi_{1}(x_{n}) & \varphi_{2}(x_{n}) & \ldots & \varphi_{q+4}(x_{n}) \end{bmatrix} \nonumber
\end{equation}
...
* Let us consider the bone data again. 
...
in between the smallest and largets $x_{i}$ values.
...
* Motivation for smoothing splines can come from considering the following minimization problem:
    + Suppose we want to find the function $m(x)$ which solves the optimization problem: 
\begin{equation}
...
* It is possible to show that the function $\hat{m}_{\lambda}(x)$ which minimizes () is 
a cubic spline with 
