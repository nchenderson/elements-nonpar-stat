...
<p class="caption">(\#fig:unnamed-chunk-1)Histogram of simulated values of exp(sample mean) with density of the Normal approximation overlaid. This assumes n=50 and that the data are from a Logistic distribution with mu = 2 and s = 2.</p>
</div>
...
* Smoothness of variance functional.
...
* In cases with smaller sample sizes. 
    + For small sample sizes, $F_{\hat{\varphi}}$ is often a better estimate than $\hat{F}_{n}$. 
...
* For cases with non-i.i.d. data, a parametric bootstrap can be easier to work with. 
...
* **Exercise 9.?** With the bootstrap we generate bootstrap samples
by sampling from $\hat{F}_{n}$. An alternative approach is to
sample from a smoother estimate of $F$ instead of the non-smooth estimate $\hat{F}_{n}$.
Consider the following estimate of $F$ which is just the indefinite integral of a 
kernel density estimate
\begin{equation}
\hat{F}_{h}^{KD}(x) = \frac{1}{n} \sum_{i=1}^{n} \int_{-\infty}^{x} \frac{1}{h} K\Big( \frac{t - X_{i}}{h} \Big) dt  \nonumber 
\end{equation}
    ...
    + As $h$ gets closer to $0$, does $\hat{F}_{h}^{KD}$ get closer to $\hat{F}_{n}$ or move further away?
