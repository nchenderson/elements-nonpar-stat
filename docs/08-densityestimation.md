...
* Remember that $x$ is a point within bin $A_{h_{n}, x_{0}}(x)$. So, we can write $x$ as
\begin{equation}
x = x_{0} + (A_{h_{n}, x_{0}}(x) - 1)h_{n} + c_{x}h_{n},
\end{equation}
where $c_{x}$ is a number such that $0 \leq c_{x} < 1$.

* So, we can simplify the expression for $p_{h_{n}, x_{0}}(x)/h_{n}$ in \@ref(eq:approx-p-hist) to
\begin{equation}
\frac{ p_{h_{n}, x_{0}}(x) }{ h_{n} } \approx f(x) + f'(x)h_{n}\Big( \frac{1}{2} - c_{x} \Big).
\end{equation}


* Hence, the approximate bias of the histogram density estimate $\hat{f}_{h_{n}}^{H}(x)$ is
\begin{eqnarray}
\textrm{Bias}\{ \hat{f}_{h_{n}}^{H}(x) \} 
&=& \frac{ p_{h_{n}, x_{0}}(x) }{ h_{n} } - f(x)  \nonumber \\
&\approx& f'(x)h_{n}\Big( \frac{1}{2} - c_{x} \Big)
(\#eq:approx-bias-hist)
\end{eqnarray}
...
&\approx& \frac{1}{nh_{n}}\Big( f(x) + f'(x)h_{n}\{ 1/2 - c_{x} \} \Big)\{1 - p_{h_{n}, x_{0}}(x) \}  \nonumber \\
&\approx& \frac{f(x)}{n h_{n} }, \textrm{ for large n}.
(\#eq:approx-var-hist)
\end{eqnarray}
...
&\approx&  [f'(x)]^{2}h_{n}^{2}\Big( \frac{1}{2} - c_{x} \Big)^{2} + \frac{f(x)}{n h_{n} } \nonumber \\
