...
Hence,
\begin{equation}
\sqrt{n}(S_{n}/n - p(\theta)) \longrightarrow \textrm{Normal}\Big( 0, p(\theta)(1 - p(\theta)) \Big) \quad \textrm{as } n \longrightarrow \infty
(\#eq:approx-signstat)
\end{equation}
...
* Notice that the power of the test depends more directly on the term $p(\delta)$. Recall from Section \@ref(sign-test) that
...
**Exercise ?**: Derive the formula for $c_{\alpha, n}$ shown in \@ref(eq:critical-value-signstat).
...
be tough to interpret (no test will be uniformly more powerful for al distributions) 
