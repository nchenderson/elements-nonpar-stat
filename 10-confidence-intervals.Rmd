...
```{r, echo=FALSE, fig.height=5.5, fig.cap="Bootstrap confidence intervals for the density function at the points x=100, 125, 150, 175"}
kk <- density(framingham$sysBP)
tmp <- approxfun(kk$x, kk$y)
fhat <- c(tmp(100), tmp(125), tmp(150), tmp(175))
plot(kk, lwd=2, las=1, xlab="Sys BP", ylab="Density", ylim=c(0,.024),
     main="Bootstrap Confidence Intervals for Systolic Blood Pressure", xlim=c(75,275))
lines(c(100, 100), c(fhat[1] - 1.96*bb[1], fhat[1] + 1.96*bb[1]), lwd=3, col="red")
lines(c(125, 125), c(fhat[2] - 1.96*bb[2], fhat[2] + 1.96*bb[2]), lwd=3, col="red")
lines(c(150, 150), c(fhat[3] - 1.96*bb[3], fhat[3] + 1.96*bb[3]), lwd=3, col="red")
lines(c(175, 175), c(fhat[4] - 1.96*bb[4], fhat[4] + 1.96*bb[4]), lwd=3, col="red")
```
...
hist(eta.boot, col="grey", las =1, xlab=expression(hat(eta)), main=expression(paste("Bootstrap replications of ", hat(eta)," for the shifted Exponential distribution")))
```
