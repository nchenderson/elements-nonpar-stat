...
hist(kidney$age, main="Histogram of Age in Kidney Data", las=1, xlab="age",
     breaks=12, col="lightgrey")

ff <- density(kidney$age)
tt <- seq(0, 100, length.out=500)
hist(kidney$age, main="Histogram of Age and Other Density Estimates", las=1, 
     xlim=c(0,100), las=1, xlab="age", ylab="Density", prob=TRUE, breaks=12,
     col="lightgrey")
...
```{r, echo=FALSE, fig.cap="Bootstrap distribution of the sample standard deviation for the age variable from the kidney fitness data. Dashed vertical lines are placed at the 2.5 and 97.5 percentiles of the bootstrap distribution."}
...
```{r, echo=FALSE, cache=TRUE}
par(xpd=NA)
RCTdata <- read.csv("~/Library/Mobile Documents/com~apple~CloudDocs/Documents/STAT685Notes/Data/RCTdata.csv")
